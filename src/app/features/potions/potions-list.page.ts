...
export class PotionsListPage {
  ...
  protected readonly difficulty = signal<string | null>(null);
  protected readonly sortValue = signal('name');
  protected readonly total = signal<number | null>(null);

  protected readonly hasActiveFilters = computed(
    () => this.difficulty() !== null || this.search.value.trim().length > 0,
  );
  ...
  protected selectDifficulty(difficulty: string | null): void {
    this.difficulty.set(this.difficulty() === difficulty ? null : difficulty);
    this.reload();
  }
  ...
  private fetch(reset: boolean): void {
    ...
    if (this.difficulty()) {
      query.filters!['difficulty_cont'] = this.difficulty();
    }
